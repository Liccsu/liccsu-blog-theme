...
(function (window, document) {
  ...
  SF.initArticleHeatmap = function () {
    ...
    const posts = window.aboutPagePosts.filter(function (p) {
    ...
    for (let wk = 0; wk < wks; wk++) {
      ...
      if (mon !== lastMon) {
        mLabels.push(
          '<text x="' + (lw + wk * (cs + cg)) + '" y="10" class="heatmap-month-label">' + mons[mon] + "</text>",
        );
        lastMon = mon;
      }
    }
    ...
    for (let idx = 0; idx < wks * dys; idx++) {
      ...
      cells.push(
        '<rect x="' +
          ...
          '" rx="2" fill="' +
          clr +
          '" class="article-heatmap-cell heatmap-cell-animated" style="animation-delay:' +
          idx +
          'ms" data-date="' +
          key +
          '" data-count="' +
    ...
    canvas.innerHTML =
      '<svg class="article-heatmap-svg" viewBox="0 0 ' +
      svgW +
      " " +
      svgH +
      '"><g>' +
    ...
    canvas.querySelectorAll(".article-heatmap-cell").forEach(function (cell) {
      cell.addEventListener("mouseenter", function (evt) {
        ...
        const dayPosts = posts.filter(function (p) {
          return new Date(p.date).toDateString() === dt.toDateString();
        });

        let htm =
          '<div class="heatmap-tooltip-date">' +
          fmt +
          '</div><div class="heatmap-tooltip-count">' +
          cnt +
          " 篇文章</div>";
        if (dayPosts.length > 0) {
          htm += '<div class="heatmap-tooltip-posts">';
          dayPosts.slice(0, 3).forEach(function (p) {
            htm += '<span class="heatmap-tooltip-post">' + p.title + "</span>";
          });
          if (dayPosts.length > 3)
            htm += '<span class="heatmap-tooltip-post">+' + (dayPosts.length - 3) + " 更多...</span>";
          htm += "</div>";
        }
  ...
  SF.initGitHub = function () {
    ...
    if (statsImg) {
      var statsUrl = cfg.statsHost + "/api?username=" + cfg.username + "&show_icons=true&hide_border=true" + locale;
    ...
    if (langsImg) {
      var langsUrl =
        cfg.statsHost + "/api/top-langs/?username=" + cfg.username + "&layout=compact&hide_border=true" + locale;

      if (hasCustomTheme) {
        langsUrl += "&theme=" + themeValue;
      } else {
        langsUrl +=
          "&theme=transparent" + "&title_color=" + primaryHex + "&text_color=" + textHex + "&bg_color=00000000";
      }
  ...
  SF.initLocation = function () {
    ...
    if (isNaN(lat) || isNaN(lng)) {
      mapEl.innerHTML =
        '<div style="display:flex;align-items:center;justify-content:center;height:100%;opacity:0.5;">请配置位置坐标</div>';
      return;
    }
    ...
    mapEl.innerHTML =
      ...
      'style="display:block;position:relative;width:100%;height:100%;min-height:' +
      height +
      'px;border-radius:0.75rem;overflow:hidden;cursor:pointer;transition:all 0.3s ease;">' +
      '<img src="' +
      staticMapUrl +
      '" alt="地图位置" ' +
